...
public class GymnPage implements Initializable {
    ...
    void refreshPage(ActionEvent event) {
        ...
        try{
            ...
            String sqlstetment = "select COURSE.CourseNumber,COURSE.CourseName,COURSE.CourseCost,COURSE.CourseType,COURSE.CourseFor,COURSE.TimeInterval,EMPLOYEE.FIRSTNAME,EMPLOYEE.LASTNAME,COURSE.NumOfStudents,COURSE.MaxNumOfStudent from COURSE,TRAINER,GIVES,employee where COURSE.COURSENUMBER=GIVES.COURSENUMBER and COURSE.COURSETYPE='Gymnastics'and EMPLOYEE.EMPLOYEESSN=TRAINER.TRAINERSSN and TRAINER.TRAINERSSN=GIVES.TRAINERSSN";
            ResultSet rs =stat.executeQuery(sqlstetment);
            int i=0;
            list.clear();;
            while(rs.next()){
                Course obj =new Course(rs.getInt(1),rs.getString(2),rs.getString(6),rs.getInt(3),9,10,rs.getString(5),rs.getString(4));
    ...
    @FXML
    void addSCourse(ActionEvent event) {
        AddStatus.setText("");
        int Sssn=0;
